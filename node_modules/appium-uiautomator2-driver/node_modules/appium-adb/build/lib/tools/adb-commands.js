...
methods.setWifiAndData = async function setWifiAndData({ wifi, data }, isEmulator = false) {
    if (support_1.util.hasValue(wifi)) {
        await this.setWifiState(wifi, isEmulator);
    }
    if (support_1.util.hasValue(data)) {
...
methods.androidCoverage = async function androidCoverage(instrumentClass, waitPkg, waitActivity) {
    ...
    return await new bluebird_1.default(async (resolve, reject) => {
        ...
        logger_js_1.default.debug(`Collecting coverage data with: ${[this.executable.path].concat(args).join(' ')}`);
        try {
            // am instrument runs for the life of the app process.
            this.instrumentProc = new teen_process_1.SubProcess(this.executable.path, args);
